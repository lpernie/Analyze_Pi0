...
// $Id: NewPi0Dumper_Gun.cc,v 1.20 2012/09/25 09:16:31 lpernie Exp $
//
//
...
class NewPi0Dumper_Gun : public edm::EDAnalyzer {
   ...
   private:
      ...
      void AssociateGammaES(std::vector<CaloCluster> clustersEE, std::vector<CaloCluster> clustersES, std::vector<GlobalPoint> posES, std::vector<float> Es_e1, std::vector<float> Es_e2,
                             std::vector< int > Ncristal_EE, std::vector< float > s4s9_EE, std::vector< float > s1s9_EE, std::vector< float > s2s9_EE);
...
NewPi0Dumper_Gun::NewPi0Dumper_Gun(const edm::ParameterSet& ps)
{
  ...
  s4s9CluCut_           = ps.getUntrackedParameter<double>("s4s9CluCut", 0.8);
...
void NewPi0Dumper_Gun::beginJob()
{
  ...
  m_EEMva  = new TTree("h_EEmva","EE Conteinment Correction");
  m_EEMva->Branch("MVA_EPi0",&MVA_EPi0,"MVA_EPi0/F");
  m_EEMva->Branch("MVA_E3x3_1",&MVA_E3x3_1,"MVA_E3x3_1/F");
  m_EEMva->Branch("MVA_E3x3MC_1",&MVA_E3x3MC_1,"MVA_E3x3MC_1/F");
  m_EEMva->Branch("MVA_E3x3_2",&MVA_E3x3_2,"MVA_E3x3_2/F");
  m_EEMva->Branch("MVA_E3x3MC_2",&MVA_E3x3MC_2,"MVA_E3x3MC_2/F");
  m_EEMva->Branch("MVA_Pt_1",&MVA_Pt_1,"MVA_Pt_1/F");
  m_EEMva->Branch("MVA_Pt_2",&MVA_Pt_2,"MVA_Pt_2/F");
  m_EEMva->Branch("MVA_Nxtal_1",&MVA_Nxtal_1,"MVA_Nxtal_1/I");
  m_EEMva->Branch("MVA_Nxtal_2",&MVA_Nxtal_2,"MVA_Nxtal_2/I");
  m_EEMva->Branch("MVA_S4S9_1",&MVA_S4S9_1,"MVA_S4S9_1/F");
  m_EEMva->Branch("MVA_S4S9_2",&MVA_S4S9_2,"MVA_S4S9_2/F");
  m_EEMva->Branch("MVA_S1S9_1",&MVA_S1S9_1,"MVA_S1S9_1/F");
  m_EEMva->Branch("MVA_S1S9_2",&MVA_S1S9_2,"MVA_S1S9_2/F");
  m_EEMva->Branch("MVA_S2S9_1",&MVA_S2S9_1,"MVA_S2S9_1/F");
  m_EEMva->Branch("MVA_S2S9_2",&MVA_S2S9_2,"MVA_S2S9_2/F");
  m_EEMva->Branch("MVA_iX_1",&MVA_iX_1,"MVA_iX_1/I");
  m_EEMva->Branch("MVA_iX_2",&MVA_iX_2,"MVA_iX_2/I");
  m_EEMva->Branch("MVA_iY_1",&MVA_iY_1,"MVA_iY_1/I");
  m_EEMva->Branch("MVA_iY_2",&MVA_iY_2,"MVA_iY_2/I");
  m_EEMva->Branch("MVA_ES1_1",&MVA_ES1_1,"MVA_ES1_1/F");
  m_EEMva->Branch("MVA_ES1_2",&MVA_ES1_2,"MVA_ES1_2/F");
  m_EEMva->Branch("MVA_ES2_1",&MVA_ES2_1,"MVA_ES2_1/F");
  m_EEMva->Branch("MVA_ES2_2",&MVA_ES2_2,"MVA_ES2_2/F");
  m_EEMva->Branch("MVA_nEta_1",&MVA_nEta_1,"MVA_nEta_1/F");
  m_EEMva->Branch("MVA_nEta_2",&MVA_nEta_2,"MVA_nEta_2/F");
  m_EEMva->Branch("MVA_nPhi_1",&MVA_nPhi_1,"MVA_nPhi_1/F");
  m_EEMva->Branch("MVA_nPhi_2",&MVA_nPhi_2,"MVA_nPhi_2/F");
  m_EEMva->Branch("MVA_isConv_1",&MVA_isConv_1,"MVA_isConv_1/O");
  m_EEMva->Branch("MVA_isConv_2",&MVA_isConv_2,"MVA_isConv_2/O");
  m_EEMva->Branch("EtaRing_1",&EtaRing_1,"EtaRing_1/I");
  m_EEMva->Branch("EtaRing_2",&EtaRing_2,"EtaRing_2/I");

  m_Mva  = new TTree("h_mva","Conteinment Correction");
  m_Mva->Branch("MVA_EPi0",&MVA_EPi0,"MVA_EPi0/F");
...
//for(unsigned int d=0; d<Nclus_ieta_EB.size(); d++) cout<< Nclus_ieta_EB.size()-EtSeed_EB.size()<<"  "<<Nclus_ieta_EB[d]<<endl;
	...
	const EERecHitCollection* hitsEE = eeHandle.product();
     ...
     PreshowerTools esClusteringAlgo(geom_, estopology_, esHandle);
     ...
     for( std::vector<CaloCluster>::const_iterator eeclus_iter  = clustersEE.begin(); eeclus_iter != clustersEE.end(); ++eeclus_iter, i++ ){
       ...
       if ((tmp1.rawId()!=0) && (tmp2.rawId()!=0) && fabs(point.eta())>1.656 && fabs(point.eta()<2.59 ) )//@ fiducial cut
       {
...
//           Added_One=true;
             ...
             if( fabs(preshowerclusterp1.get_z())>30  && fabs(preshowerclusterp2.get_z())>30 ){
                ...
                eseeclusters.push_back( CaloCluster( tempenergy, posit, CaloID(CaloID::DET_ECAL_ENDCAP),  eeclus_iter->hitsAndFractions(), CaloCluster::undefined, eeclus_iter->seed() ) );
               ...
               GlobalPoint posFak(-999.,-999.,-999.); posES.push_back(posFak);
              }
           }
           else{
             ...
             Es_e1.push_back(-999); Es_e2.push_back(-999);
             GlobalPoint posFak(-999.,-999.,-999.); posES.push_back(posFak);
           }
        }
        else{
   ...
   if( eseeclusters.size()!=Es_e1.size() || eseeclusters.size()!=clustersEE.size() ) cout<<Es_e1.size()<<"  "<<Es_e2.size()<<"<-check  ok->"<<eseeclusters.size()<<"  "<<clustersEE.size()<<endl;
   if( eseeclusters.size()!=posES.size() || posES.size()!=Ncristal_EE.size() )       cout<<posES.size()<<"  "<<Es_e2.size()<<"<-check posES  ok->"<<eseeclusters.size()<<"  "<<Ncristal_EE.size()<<endl;

   if(storeMCTruth_ && gamma1.IsGoodPi0 && clustersEE.size()>0) AssociateGammaES(clustersEE, eseeclusters, posES, Es_e1, Es_e2, Ncristal_EE, s4s9_EE, s1s9_EE, s2s9_EE);
...
if( clustersEE.size()!=Ncristal_EE.size() || clustersEE.size()!=s4s9_EE.size() || clustersEE.size()!=s1s9_EE.size() || clustersEE.size()!=s2s9_EE.size() )
  cout<<"-> EE Variables size. Clu: "<<clustersEE.size()<<" xtal: "<<Ncristal_EE.size()<<" S4: "<<s4s9_EE.size()<<" S1: "<<s1s9_EE.size()<<" S2: "<<s2s9_EE.size()<<endl;
     ...
     for(unsigned int i=0; i<clustersEE.size(); i++){
       ...
       s4s9_all.push_back( s4s9_EE[i] ); s1s9_all.push_back( s1s9_EE[i] ); s2s9_all.push_back( s2s9_EE[i] );
     }
...
void NewPi0Dumper_Gun::AssociateGamma(std::vector<CaloCluster> clusters,  std::vector< int > AllCristal, std::vector< float > s4s9_all, std::vector< float > s1s9_all, std::vector< float > s2s9_all )
{
  ...
  if( assoc && assoc2){
    ...
    if( (g1.seed().subdetId()==1) && (g2.seed().subdetId()==1) ){
       ...
       Nfinal_EB++;
       MVA_EPi0        = (G_Sort_1+G_Sort_2).E()/cosh((G_Sort_1+G_Sort_2).Eta());
...
#ifdef TTREEMVA
    int Ind1 = n_tmp, Ind2 = n_tmp2;
    const CaloCluster g1 = clustersES[Ind1];
    const CaloCluster g2 = clustersES[Ind2];
    int n_tmp_tmp = -1, iEta_tmp = 0, iPhi_tmp = 0;
    TLorentzVector G_Sort_1, G_Sort_2;
    float e3x3MC_1 = -1, e3x3MC_2 = -1;
    bool isConverted_1 = gamma1.IsConverted, isConverted_2 = gamma2.IsConverted;

    if( (g1.seed().subdetId()==2) && (g2.seed().subdetId()==2) ){
       ...
       if( g1.energy()/cosh(g1.eta()) > g2.energy()/cosh(g2.eta()) ){
         ...
         e3x3MC_2 = gamma1.gamma.E();
         n_tmp_tmp = Ind1; Ind1 = Ind2;   Ind2 = n_tmp_tmp;
         iEta_tmp = ix1; ix1 = ix2; ix2 = iEta_tmp;
         iPhi_tmp = iy1; iy1 = iy2; iy2 = iPhi_tmp;
         isConverted_1 = gamma2.IsConverted; isConverted_2 = gamma1.IsConverted;
       }
       ...
       MVA_Pt_1        = G_Sort_1.Pt();
       MVA_Pt_2        = G_Sort_2.Pt();
       MVA_Nxtal_1     = Ncristal_EE[Ind1];
       MVA_Nxtal_2     = Ncristal_EE[Ind2];
       MVA_S4S9_1      = s4s9_EE[Ind1];
       MVA_S4S9_2      = s4s9_EE[Ind2];
       MVA_S1S9_1      = s1s9_EE[Ind1];
       MVA_S1S9_2      = s1s9_EE[Ind2];
       MVA_S2S9_1      = s2s9_EE[Ind1];
       MVA_S2S9_2      = s2s9_EE[Ind2];
       MVA_ES1_1       = Es_e1[Ind1];
       MVA_ES1_2       = Es_e1[Ind2];
       MVA_ES2_1       = Es_e2[Ind1];
       MVA_ES2_2       = Es_e2[Ind2];
       if(ix1<50) MVA_iX_1 = ix1;
       else       MVA_iX_1 = 100-ix1;
       if(ix2<50) MVA_iX_2 = ix2;
       else       MVA_iX_2 = 100-ix2;
       if(iy1<50) MVA_iY_1 = iy1;
       else       MVA_iY_1 = 100-iy1;
       if(iy2<50) MVA_iY_2 = iy2;
       else       MVA_iY_2 = 100-iy2;
       MVA_nEta_1      = G_Sort_1.Eta();
       MVA_nEta_2      = G_Sort_2.Eta();
       MVA_nPhi_1      = G_Sort_1.Phi();
       MVA_nPhi_2      = G_Sort_2.Phi();
       MVA_isConv_1    = isConverted_1;
       MVA_isConv_2    = isConverted_2;
       EtaRing_1 = GetRing( ix1, iy1, VectRing, false);
       EtaRing_2 = GetRing( ix2, iy2, VectRing, false);
       m_EEMva->Fill();
    }
#endif
